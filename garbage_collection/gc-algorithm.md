...
## 栈 (Stack)
32位linux的内存布局如下 ![](./image/32位内存布局.png)
64位linux的内存布局如下 ![](./image/64位内存布局.png)
...
![](./image/1280px-Breadth-first_tree.svg.png)

搜索顺序为1-2-5-9-10-6-3....，不清楚的也可以去搜索动图进行理解

当然我们这里也能使用宽度搜索BFS，搜索顺序为1 2 3 4 5 6 7 8...即先搜索了每一层的数据，再搜索下一层的数据
