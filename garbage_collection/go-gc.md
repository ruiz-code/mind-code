...
(2) 当被标记为灰色的时候，灰色的内存变量不使用的时候，当然这个失误可以在下一次gc过程中被检测出来 (这一点其实不管也行，下次gc的时候就自然之道了)
...
1. gc的工作是使用25%的cpu(GOGC)进行清理的，所以有可能清理的速度比产生垃圾的速度慢，所以这里需要程序自己编写清理的机制，或者看看最新的golang是否有新的控制的方式。
